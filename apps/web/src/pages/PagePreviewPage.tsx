...
function PreviewWidgetContent({ widget, data, contentH = 200 }: { widget: any; data: any; contentH: number }) {
  ...
  const previewClockOffset = previewDevice?.clockOffsetMin ?? 0;
  ...
  if (widget.type === 'line_chart') {
    ...
    return pts.length > 0
      ? <div style={{ padding: '4px 0', height: '100%' }}><LineChart series={[{ name: widget.field ?? '', data: pts, color: 'hsl(var(--primary))' }]} height={chartH} showArea storedTz={storedTz} displayTz={displayTz} clockOffsetMin={previewClockOffset} /></div>
      : empty('No data yet');
  }
  ...
  if (widget.type === 'multi_line_chart') {
    ...
    return <div style={{ padding: '4px 0', height: '100%' }}><LineChart series={chartSeries} height={chartH} storedTz={storedTz} displayTz={displayTz} clockOffsetMin={previewClockOffset} /></div>;
  }
  ...
  if (widget.type === 'text') {
    ...
    return (
      <div style={{ padding: pad, height: '100%', overflowY: 'auto', display: 'flex', alignItems: 'center', boxSizing: 'border-box' }}>
        <div style={{ fontFamily: isDisplay ? 'var(--font-display)' : 'var(--font-mono)', fontSize, color, lineHeight: 1.45, textAlign: align as React.CSSProperties['textAlign'], whiteSpace: 'pre-wrap', width: '100%' }}>
          {text}
        </div>
      </div>
    );
