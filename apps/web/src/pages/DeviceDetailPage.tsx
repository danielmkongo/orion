...
export function DeviceDetailPage() {
  ...
  const { data: savedCodes, refetch: refetchSavedCodes } = useQuery({
    queryKey: ['generated-codes', id],
    queryFn: () => apiClient.get('/generated-codes', { params: { deviceId: id } }).then(r => r.data as any[]),
    enabled: !!id,
  });

  const { data: openCode } = useQuery({
    queryKey: ['generated-code', openCodeId],
    queryFn: () => apiClient.get(`/generated-codes/${openCodeId}`).then(r => r.data),
    enabled: !!openCodeId,
  });
  ...
  return (
    <div className="page">
      ...
      {/* ── Section VII: Saved Code ── */}
      {savedCodes && savedCodes.length > 0 && (
        <div className="section">
          <div>
            <div className="ssh">Saved code</div>
            <p className="dim" style={{ fontSize: 13, marginTop: 8, maxWidth: '28ch' }}>
              Firmware snippets generated by Orion AI for this device.
            </p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {savedCodes.map((item: any) => (
              <div key={item._id} className="panel" style={{ border: '1px solid hsl(var(--border))' }}>
                {/* Header row */}
                <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <FileCode size={14} style={{ color: 'hsl(var(--primary))', flexShrink: 0 }} />
                  ...
                  <button
                    ...
                    onClick={() => {
                      setOpenCodeId(v => v === item._id ? null : item._id);
                      setCodeEditMode(false);
                    }}
                  >
                    {openCodeId === item._id ? 'Close' : 'View'}
                  </button>
                  <button
                    className="btn btn-ghost btn-sm btn-icon"
                    onClick={async () => {
                      if (!window.confirm(`Delete "${item.name}"?`)) return;
                      await apiClient.delete(`/generated-codes/${item._id}`);
                      toast.success('Deleted');
                      refetchSavedCodes();
                      if (openCodeId === item._id) setOpenCodeId(null);
                    }}
                  >
                    <Trash2 size={12} />
                  </button>
                ...
                {/* Expanded code view */}
                {openCodeId === item._id && openCode && (
                  <div style={{ borderTop: '1px solid hsl(var(--border))' }}>
                    {/* Toolbar */}
                    <div style={{ padding: '8px 14px', display: 'flex', gap: 6, alignItems: 'center', background: 'hsl(var(--surface-raised))' }}>
                      <button
                        className="btn btn-ghost btn-sm btn-icon"
                        style={{ color: codeEditMode ? 'hsl(var(--primary))' : undefined }}
                        onClick={() => { setCodeEditMode(v => !v); setCodeDraft(openCode.code); }}
                        title={codeEditMode ? 'Preview' : 'Edit'}
                      >
                        <Pencil size={11} />
                      </button>
                      <button className="btn btn-ghost btn-sm btn-icon" onClick={() => copyText(codeEditMode ? codeDraft : openCode.code).then(() => toast.success('Copied!'))} title="Copy">
                        <Copy size={11} />
                      </button>
                      <button className="btn btn-ghost btn-sm btn-icon" onClick={() => {
                        const blob = new Blob([codeEditMode ? codeDraft : openCode.code], { type: 'text/plain' });
                        const url  = URL.createObjectURL(blob);
                        const a    = document.createElement('a');
                        a.href = url; a.download = `${item.name.toLowerCase().replace(/\s+/g, '_')}.${item.protocol === 'pico' ? 'py' : 'ino'}`; a.click();
                      ...
                      {codeEditMode && (
                        <button
                          ...
                          onClick={async () => {
                            setCodeSaving(true);
                            try {
                              await apiClient.patch(`/generated-codes/${item._id}`, { code: codeDraft });
                              queryClient.invalidateQueries({ queryKey: ['generated-code', item._id] });
                              setCodeEditMode(false);
                              toast.success('Saved!');
                        ...
                        >
                          <Save size={10} /> {codeSaving ? 'Saving…' : 'Save changes'}
                        </button>
                      )}
                    </div>
                    {codeEditMode ? (
                      <textarea
                        ...
                        spellCheck={false}
                        style={{
                          display: 'block', width: '100%', minHeight: 320, resize: 'vertical',
                          padding: '14px 16px', background: 'hsl(var(--bg))', border: 'none', outline: 'none',
                      ...
                      <pre style={{
                        ...
                        maxHeight: 400, background: 'hsl(var(--bg))',
                        fontFamily: 'var(--font-mono)', fontSize: 11, lineHeight: 1.65,
                        whiteSpace: 'pre', color: 'hsl(var(--fg))',
                      }}>
                        {openCode.code}
                      </pre>
                    )}
